...
import { Play, BookOpen, Clock, Download, Award, CheckCircle, Video, MessageSquare, User, FileText } from './Icons';
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem', maxHeight: '82vh', overflowY: 'auto', paddingRight: '0.25rem' }}>
        ...
        {activeTab === 'certificate' && (
          <div style={{
            ...
            <div style={{ color: '#3b82f6', fontWeight: 800, fontSize: '1.4rem', letterSpacing: '0.05em' }}>
              BK TEACHING CENTER
            </div>
            ...
            <p style={{ fontSize: '0.95rem', color: 'var(--text-muted)' }}>This certifies that</p>
            <h2 style={{ fontSize: '1.9rem', color: 'var(--text-main)', textDecoration: 'underline #3b82f6' }}>
              {user?.name || 'Valued Student'}
            </h2>
            <p style={{ fontSize: '0.95rem', color: 'var(--text-muted)', maxWidth: '540px', margin: '0 auto' }}>
              has successfully completed all required modules, assignments, and practical assessments for
            </p>
            <h3 style={{ fontSize: '1.3rem', color: '#3b82f6', fontWeight: 800 }}>{course.title}</h3>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '1.5rem', paddingTop: '1rem', borderTop: '1px solid var(--border-color)', fontSize: '0.8rem', color: 'var(--text-muted)' }}>
              ...
              <span>Verified ID: BKTC-{course._id || '8842'}-CERT</span>
            </div>

            <button
              className="curious-btn-primary"
              style={{ margin: '1rem auto 0', padding: '0.6rem 1.5rem', backgroundColor: '#3b82f6', borderColor: '#3b82f6' }}
              onClick={() => alert('Certificate downloaded as High-Res PDF!')}
            >
